...
fn main() {
    // Local (non-docker) build: docker mode's context (airdrop/) excludes the
    ...
    // testnet's per-tx cap. The on-chain image_id changes (the ELF header bytes
    // are part of the first PT_LOAD segment), but the airdrop host reads the
    // .bin file fresh at runtime via `Program::new`, so the expected program ID
    // matches the deployed bytecode automatically.
    ...
    //     DEPLOY artifact the host reads via `AIRDROP_BINARY`. This one is
